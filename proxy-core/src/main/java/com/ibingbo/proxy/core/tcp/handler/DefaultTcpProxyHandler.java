...
import com.ibingbo.proxy.core.channel.SocketChannel;

/**
 * Created by bing on 17/8/28.
 */
public class DefaultTcpProxyHandler extends Thread implements ProxyHandler {

    private Socket client;

    public DefaultTcpProxyHandler(Socket client) {
        this.client = client;
        this.start();
    }
            ...
            if (cin != null && cout != null) {
                ...
                if (creadlen > 0) { // 读到数据
                    if (creadBuf[0] == 5) { // 读到 SOCK5 请求
                        ...
                        if (creadlen > 0) { // 读到 SOCK5 请求
                            if (creadBuf[0] == 5 && creadBuf[1] == 1 && creadBuf[2] == 0 && creadBuf[3] == 1) {//TCP 请求
                                ...
                                ip = byte2int(creadBuf[4]) + "." + byte2int(creadBuf[5]) + "." + byte2int(
                                        creadBuf[6])
                                        + "." + byte2int(creadBuf[7]);
                    ...
                    if (creadBuf[0] == 4) { // 读到 SOCK4 请求
                        ...
                        if (creadBuf[4] == 0 && creadBuf[5] == 0 && creadBuf[6] == 0 && creadBuf[7] != 0
                                && creadBuf[8] == 0) {
                            // 如请求中为域名
                            ...
                            ip = byte2int(creadBuf[4]) + "." + byte2int(creadBuf[5]) + "." + byte2int(creadBuf[6])
                                    + "."
                                    + byte2int(creadBuf[7]);
                            s = ip;
                        }
